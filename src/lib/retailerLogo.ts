...
export async function fetchUserIdByRetailerId(retailerId: number): Promise<number | null> {
  try {
    ...
    const url = `${API_CONFIG.BASE_URL}/api/users:list?pageSize=20&page=1&sort[]=-updatedAt&filter=${filter}&fields[]=id`;
...
export async function fetchRetailerLogoUrlByUserId(userId: number): Promise<string | null> {
  try {
    const url = `${API_CONFIG.BASE_URL}/api/users:get?filterByTk=${encodeURIComponent(String(userId))}&fields[]=id&fields[]=retailer_logo`;
    const res = await fetch(url, { headers: getAuthHeaders() });
    if (!res.ok) return null;
    const json = await res.json();
